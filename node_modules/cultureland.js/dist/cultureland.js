...
export default class Cultureland {
     ...
     */
    async checkVoucher(pin) {
        if (!(await this.isLogin()))
            throw new CulturelandError("LoginRequiredError", "로그인이 필요한 서비스 입니다.");
        ...
        const voucherData = voucherDataRequest.data;
        if (voucherData.resultCd === "0") {
            const resultOther = JSON.parse(voucherData.resultOther);
            return {
    ...
    async charge(pins) {
        ...
        if (pins instanceof Pin) { // 핀번호가 1개라면
            ...
            const chargePageRequest = await this.client.get(pin.parts[3].length === 4 ?
            ...
            if (chargePageRequest instanceof AxiosError) {
                return new CulturelandError("AxiosError", `GET ${pin.parts[3].length === 4 ?
                    "/csh/cshGiftCard.do" :
                    "/csh/cshGiftCardOnline.do"} 요청에 실패하였습니다.`, chargePageRequest);
            }
            const transKey = new mTransKey(this.cookieJar);
            const servletData = await transKey.getServletData();
            ...
            const chargeRequest = await this.client.post(pin.parts[3].length === 4 ?
            ...
            if (chargeRequest instanceof AxiosError) {
                return new CulturelandError("AxiosError", `POST ${pin.parts[3].length === 4 ?
                    "/csh/cshGiftCardProcess.do" :
                    "/csh/cshGiftCardOnlineProcess.do"} 요청에 실패하였습니다.`, chargeRequest);
            }
            ...
            const chargeResult = chargeResultRequest.data; // 충전 결과 받아오기
            ...
            // 선행 페이지 요청을 보내지 않으면 잘못된 접근 오류 발생
            const chargePageRequest = await this.client.get(onlyMobileVouchers ?
                "https://m.cultureland.co.kr/csh/cshGiftCard.do" : // 모바일문화상품권
                "https://m.cultureland.co.kr/csh/cshGiftCardOnline.do" // 문화상품권(18자리)
            ); // 문화상품권(18자리)에서 모바일문화상품권도 충전 가능, 모바일문화상품권에서 문화상품권(18자리) 충전 불가능
            if (chargePageRequest instanceof AxiosError) {
                return new CulturelandError("AxiosError", `GET ${onlyMobileVouchers ?
                    "/csh/cshGiftCard.do" :
                    "/csh/cshGiftCardOnline.do"} 요청에 실패하였습니다.`, chargePageRequest);
            }
            const transKey = new mTransKey(this.cookieJar);
            ...
            const scratches = {}; // scratch (핀번호)
            const keyboards = []; // keyIndex, keyboardType, fieldType
            const transkeys = []; // transkey, transkey_HM
            for (const pin of pins) {
                if (pin.parts === null)
                    continue; // 유효하지 않은 핀번호의 경우 건너뛰기, 위에서 이미 확인하여 의미 없지만 린팅 이슈로 추가
            ...
            const chargeRequest = await this.client.post(onlyMobileVouchers ?
                "https://m.cultureland.co.kr/csh/cshGiftCardProcess.do" : // 모바일문화상품권
                "https://m.cultureland.co.kr/csh/cshGiftCardOnlineProcess.do", // 문화상품권(18자리)
            ...
            if (chargeRequest instanceof AxiosError) {
                return new CulturelandError("AxiosError", `POST ${onlyMobileVouchers ?
                    "/csh/cshGiftCardProcess.do" :
                    "/csh/cshGiftCardOnlineProcess.do"} 요청에 실패하였습니다.`, chargeRequest);
            }
            const chargeResultRequest = await this.client.get("https://m.cultureland.co.kr" + chargeRequest.headers.location);
    ...
    async gift(amount, quantity = 1) {
        ...
        const phoneInfo = phoneInfoRequest.data;
        ...
        const giftResult = giftResultRequest.data; // 선물 결과 받아오기
        ...
        if (giftResult.includes("<strong> 컬쳐랜드상품권(모바일문화상품권) 선물(구매)가<br />완료되었습니다.</strong>")) {
            ...
            const barcodeData = barcodeDataRequest.data;
            // 선물 결과에서 핀번호(바코드 번호) 파싱
            const pinCode = barcodeData
    ...
    async getGiftLimit() {
        ...
        const limitInfo = limitInfoRequest.data;
     ...
     */
    async changeCoupangCash(amount) {
        if (!(await this.isLogin()))
            throw new CulturelandError("LoginRequiredError", "로그인이 필요한 서비스 입니다.");
        // 전환 금액이 조건에 맞지 않을 때
        ...
        const changeCoupangPage = changeCoupangPageRequest.data;
        // 컬쳐캐쉬 캐시백 파라미터
        ...
        const changeCoupangCash = changeCoupangCashRequest.data;
    ...
    async changeSmileCash(amount) {
        ...
        const changeSmileCashPage = changeSmileCashPageRequest.data;
        // 컬쳐캐쉬 캐시백 파라미터
        ...
        const changeSmileCash = changeSmileCashRequest.data;
     ...
     */
    async giftGooglePlay(amount, quantity = 1) {
        if (!(await this.isLogin()))
            throw new CulturelandError("LoginRequiredError", "로그인이 필요한 서비스 입니다.");
        // 구매 금액이 조건에 맞지 않을 때
        ...
        const googlePage = googlePageRequest.data;
        // 컬쳐랜드 파라미터
        ...
        const oldGoogleHistory = oldGoogleHistoryRequest.data;
        const payload = [
            ["tfsSeq", tfsSeq],
        ...
        const sendGoogle = sendGoogleRequest.data;
        ...
        let googleHistory = googleHistoryRequest.data;
        ...
        // 구매 전 기프트 코드 내역과 대조
        const googleGifts = googleHistory.list.filter(history => parseInt(history.item.Amount) === amount &&
    ...
    async getMemberInfo() {
        ...
        const memberInfo = memberInfoRequest.data;
     ...
     */
    async getCultureCashLogs(days, pageSize = 20, page = 1) {
        if (!(await this.isLogin()))
            throw new CulturelandError("LoginRequiredError", "로그인이 필요한 서비스 입니다.");
        ...
        const cashLogs = cashLogsRequest.data;
    ...
    async isLogin() {
        ...
        if (isLoginRequest instanceof AxiosError)
            return false;
     ...
     */
    async login(id, password, captchaKey = null, browserId = null, macAddress = null) {
        // 브라우저 아이디가 없을 때
        if (browserId === null) {
        ...
        if (captchaKey === null) { // 캡챠키를 입력하지 않았을 경우
            ...
            const randomString = new Array(64).fill("").map(() => "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789".charAt(Math.random() * 62)).join("");
            // KeepLoginConfig 쿠키를 사용할 경우 hCaptcha 값의 유효 여부를 확인하지 않는 취약점 사용
            await this.cookieJar.setCookie(new Cookie({ key: "KeepLoginConfig", value: randomString }), "https://m.cultureland.co.kr");
        }
        else {
        ...
        if (loginRequest.headers.location === "/cmp/authConfirm.do") {
            ...
            const errorPage = errorPageRequest.data;
            // 제한코드 가져오기
     ...
     * @returns `macAddress`: string - 임의의 MAC 주소
     */
    async loginWithKeepLoginInfo(keepLoginInfo, browserId = null, macAddress = null) {
        // 로그인 유지 정보를 쿠키에 저장
        await this.cookieJar.setCookie(new Cookie({ key: "KeepLoginConfig", value: encodeURIComponent(keepLoginInfo) }), "https://m.cultureland.co.kr");
        ...
        const loginMain = loginMainRequest.data;
