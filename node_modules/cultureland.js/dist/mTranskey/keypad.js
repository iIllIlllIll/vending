...
export default class Keypad {
    ...
    crypto;
    keyIndex;
    constructor(servletData, cookieJar, keyboardType, name, inputName, fieldType, crypto) {
     ...
     * @returns 암호화된 비밀번호
     */
    encryptPassword(pw, layout) {
        let encrypted = "";
        for (const val of pw.split("")) {
            const geo = this.servletData.keyInfo[this.keyboardType][layout.indexOf(specialChars.includes(val) ? // val이 특수문자라면
            ...
            if (!geo)
                throw new Error("ERROR_GEO_NOT_FOUND"); // 키패드에 존재하지 않는 키
            let geoString = geo.join(" ");
            if (this.keyboardType === "qwerty") { // qwerty 키보드라면
                if (specialChars.includes(val))
                    geoString = "s " + geo.join(" "); // 특수문자라면
                else if (val === val.toUpperCase())
                    geoString = "u " + geo.join(" "); // 대문자라면
                else
                    geoString = "l " + geo.join(" "); // 소문자 또는 숫자라면
            }
    ...
    async getKeypadLayout() {
        ...
        for (let y = 0; y < (this.keyboardType === "qwerty" ? 4 : 3); y++) { // 키패드 세로 칸만큼 반복
            for (let x = 0; x < (this.keyboardType === "qwerty" ? 11 : 4); x++) { // 키패드 가로 칸만큼 반복
                ...
                const img = keyImage.clone()
                    .crop(this.keyboardType === "qwerty" ? x * 54 + 22 : x * 160 + 70, // x 좌표
        ...
        for (const key of keys) {
            ...
            if (this.keyboardType === "qwerty") {
                if (keyHash === "44529f2104900879d858baf3f763b3b2")
                    layout.push(-1); // 빈 칸
                else
                    layout.push(i++);
            }
            else
                layout.push(numberKeyHashes.indexOf(keyHash)); // 사진의 해시를 이용해 어떤 키인지 찾아냄
        }
        return layout;
    }
}
